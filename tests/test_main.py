...
class TestHandleOverseasPendingOrders:
    ...
    async def test_buy_pending_wide_quote_gap_is_cancelled_in_low_liquidity_session(self) -> None:
        ...
        with patch("src.broker.pending_orders.classify_session_id", return_value="US_DAY"):
            await handle_overseas_pending_orders(
    ...
    @pytest.mark.asyncio
    async def test_buy_pending_falls_back_to_last_based_price_when_orderbook_fetch_fails(
        self,
    ) -> None:
        """BUY retry should degrade to last-price chase when orderbook fetch fails."""
        settings = self._make_settings("US_NASDAQ")
        telegram = self._make_telegram()
        ...
        overseas_broker.cancel_overseas_order = AsyncMock(return_value={"rt_cd": "0", "msg1": "OK"})
        overseas_broker.get_overseas_price = AsyncMock(return_value={"output": {"last": "200.0"}})
        overseas_broker.get_overseas_orderbook = AsyncMock(side_effect=ConnectionError("down"))
        overseas_broker.send_overseas_order = AsyncMock(return_value={"rt_cd": "0", "msg1": "OK"})

        await handle_overseas_pending_orders(overseas_broker, telegram, settings, {}, {})

        overseas_broker.send_overseas_order.assert_called_once()
        assert overseas_broker.send_overseas_order.call_args[1]["price"] == round(200.0 * 1.004, 2)
    ...
    @pytest.mark.asyncio
    async def test_sell_pending_falls_back_to_last_based_price_when_orderbook_fetch_fails(
        self,
    ) -> None:
        """SELL retry should degrade to last-price pricing when orderbook fetch fails."""
        settings = self._make_settings("US_NASDAQ")
        telegram = self._make_telegram()
        ...
        overseas_broker.send_overseas_order.assert_called_once()
        assert overseas_broker.send_overseas_order.call_args[1]["price"] == round(200.0 * 0.996, 2)

    @pytest.mark.asyncio
    async def test_overseas_unknown_exchange_uses_fallback_market_without_crash(self) -> None:
        """Unknown exchange codes should still proceed using fallback market key semantics."""
        settings = self._make_settings("US_NASDAQ")
        telegram = self._make_telegram()
        ...
        overseas_broker.get_overseas_price = AsyncMock(return_value={"output": {"last": "200.0"}})
        overseas_broker.get_overseas_orderbook = AsyncMock(
            return_value={"output2": {"pask1": "200.50", "pbid1": "199.00"}}
        )
        overseas_broker.send_overseas_order = AsyncMock(return_value={"rt_cd": "0", "msg1": "OK"})
        ...
        overseas_broker.send_overseas_order.assert_called_once()
        assert overseas_broker.send_overseas_order.call_args[1]["exchange_code"] == "XNAS"
...
class TestHandleDomesticPendingOrders:
    ...
    async def test_buy_pending_falls_back_to_last_based_price_when_orderbook_fetch_fails(
        ...
        """KR BUY retry should degrade to last-price chase when orderbook fetch fails."""
        ...
        broker.cancel_domestic_order = AsyncMock(return_value={"rt_cd": "0", "msg1": "OK"})
        broker.get_current_price = AsyncMock(return_value=(50000.0, 0.0, 0.0))
        broker.get_orderbook_by_market = AsyncMock(side_effect=ConnectionError("down"))
        broker.send_order = AsyncMock(return_value={"rt_cd": "0", "msg1": "OK"})

        await handle_domestic_pending_orders(broker, telegram, settings, {}, {})

        broker.send_order.assert_called_once()
        assert broker.send_order.call_args[1]["price"] == kr_round_down(50000.0 * 1.004)
    ...
    async def test_sell_pending_falls_back_to_last_based_price_when_orderbook_fetch_fails(
        ...
        """KR SELL retry should degrade to last-price pricing when orderbook fetch fails."""
        from src.broker.kis_api import kr_round_down
        ...
        broker.send_order.assert_called_once()
        assert broker.send_order.call_args[1]["price"] == kr_round_down(50000.0 * 0.996)
