...
@pytest.mark.asyncio
async def test_run_daily_mode_logs_actual_phase_lifecycle_without_contract_helper(
    caplog: pytest.LogCaptureFixture,
) -> None:
    class _FakeEvent:
        def __init__(self) -> None:
    ...
    reviewer.generate_lessons = AsyncMock(return_value=["actual lifecycle observed"])
    next_open_time = datetime(2026, 3, 25, 0, 0, tzinfo=UTC)
    next_batch_time = datetime(2026, 3, 24, 6, 0, tzinfo=UTC)
    wait_calls = 0
    process_daily_session_stock = AsyncMock()
    ...
    with ExitStack() as stack:
        ...
        stack.enter_context(
            patch(
                "src.main.get_open_markets",
                side_effect=[[MARKETS["KR"]], [MARKETS["KR"]], []],
            )
        )
        stack.enter_context(
            patch("src.main.get_session_info", return_value=MagicMock(session_id="KRX_REG"))
        ...
        stack.enter_context(
            patch("src.main._resolve_daily_mode_next_batch_at", return_value=next_batch_time)
        )
        stack.enter_context(
            patch(
        ...
        stack.enter_context(
            patch("src.main._process_daily_session_stock", new=process_daily_session_stock)
        )

        with caplog.at_level(logging.INFO):
            await main_module.run(settings)

    process_daily_session_stock.assert_awaited_once()
    reviewer.generate_scorecard.assert_called_once()
    reviewer.generate_lessons.assert_awaited_once()
    telegram.return_value.notify_market_close.assert_awaited_once()
    context_aggregator.return_value.aggregate_daily_from_trades.assert_called_once()
    for phase in range(7):
        assert f"daily_cycle phase={phase}" in caplog.text
    assert "step=prepare_market" in caplog.text
    assert "step=daily_review" in caplog.text
    assert "step=wait_for_market_open" in caplog.text
    assert "status=contract" not in caplog.text
