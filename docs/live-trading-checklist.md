...
- [ ] canonical 운영 프로세스 재실행은 Symphony `hooks.before_remove` 가 merged worktree 삭제 직전에 `bash scripts/symphony_before_remove_canonical_restart.sh` 를 실행해 자동 처리하는지 확인한다.
