...
- BUY execution also checks the latest SELL inside `SELL_REENTRY_PRICE_GUARD_SECONDS`; the comparison stays strict (`current_price > last_sell_price`) and does not add a fee/slippage buffer until a market-aware cost model exists.
